---
title: "Contrat d’ingestion"
description: "Le format de transport des events — la source de vérité partagée par le backend et les SDKs."
---
...
Raison technique (cf. [`architecture`](../architecture/)) : la table est partitionnée par
...
|---|---|---|---|
| `iss` | string | recommandé | émetteur (backend consommateur). Vérifié si `[token].issuer` est configuré. |
| `aud` | string | recommandé | audience, valeur attendue `datacat-ingest`. Vérifié si `[token].audience` est configuré. |
...
4. `exp` non dépassé (avec une tolérance d'horloge `[token].leeway`, défaut 60 s).
...
Deux modes, configurés dans la section `[token]` de `datacat.toml` :

- **PEM en configuration** (`public_key_pem = "${TOKEN_PUBLIC_KEY_PEM}"` ou `public_key_file`) :
  clé(s) publique(s) fournie(s) au déploiement. Rotation = ajout d'une nouvelle clé puis
  retrait de l'ancienne.
- **JWKS** (`jwks_url`) : l'ingestion récupère et met en cache le jeu de clés publiques
  du backend consommateur, rafraîchi périodiquement ; sélection par `kid`. Permet une
  rotation sans redéploiement de l'ingestion.

> En dev/test, un repli par variables d'environnement existe (`TOKEN_*`) en l'absence de
> `datacat.toml`. Voir [configuration](../configuration/).

La spécification d'émission complète (exemples de génération côté consommateur) est dans le
[contrat du token](../token/).
