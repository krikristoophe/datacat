...
// ── Variantes pour le domaine « métriques » (cf. migrations/0006_metrics.sql) ──

pub async fn ensure_metric_partition_window(
    pool: &PgPool,
    past_days: i64,
    future_days: i64,
) -> Result<()> {
    ...
    for offset in -past_days..=future_days {
        let day = today + chrono::Duration::days(offset);
        sqlx::query("SELECT datacat_ensure_metric_partition($1)")
            .bind(day)
            .execute(pool)
            .await
            .with_context(|| format!("création de la partition de métriques du {day}"))?;
    }
    Ok(())
}

pub async fn purge_old_metric_partitions(pool: &PgPool, retention_days: i64) -> Result<i64> {
    let cutoff = Utc::now().date_naive() - chrono::Duration::days(retention_days);
    let dropped: i32 = sqlx::query_scalar("SELECT datacat_drop_metric_partitions_before($1)")
        .bind(cutoff)
        .fetch_one(pool)
        .await
        .context("purge des partitions de métriques")?;
    Ok(dropped as i64)
}

pub async fn drain_metric_staging(pool: &PgPool) -> Result<u64> {
    sqlx::query("SELECT datacat_ensure_metric_partitions_for_staging()")
        .execute(pool)
        .await?;
    let inserted: i64 = sqlx::query_scalar("SELECT datacat_merge_metric_staging()")
        .fetch_one(pool)
        .await?;
    Ok(inserted.max(0) as u64)
}
