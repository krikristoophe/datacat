...
use datacat_ingest::alerting::{
    evaluate_once, parse_rules, AlertEngineState, AlertState, Notifier, RecordingNotifier,
    SlackNotifier,
};
...
/// Insère un log ERROR (severity 17) à `now` pour un service donné.
async fn insert_error_log(pool: &PgPool, service: &str) {
    ...
    sqlx::query(
        "INSERT INTO logs \
         ...
         VALUES (gen_random_uuid(), $1, now(), 17, 'ERROR', 'boom', $2, '{}'::jsonb, '{}'::jsonb)",
    )
    .bind(now)
    .bind(service)
    .execute(pool)
...
async fn metric_threshold_fires_slack(pool: PgPool) {
    ...
    let notified = evaluate_once(&pool, &rules, &mut state, &notifiers, Utc::now()).await;
...
async fn log_count_fires_and_respects_cooldown(pool: PgPool) {
    ...
    let notifiers: Vec<Arc<dyn Notifier>> = vec![Arc::new(recorder.clone())];
    let mut state = AlertEngineState::new();

    // 1re évaluation : transition ok→firing → une alerte.
    let now = Utc::now();
    let n1 = evaluate_once(&pool, &rules, &mut state, &notifiers, now).await;
    assert_eq!(n1, 1);

    // 2e évaluation immédiate (même état firing, dans le cooldown) → aucune re-notification.
    let n2 = evaluate_once(&pool, &rules, &mut state, &notifiers, now).await;
...
async fn fires_then_resolves(pool: PgPool) {
    ...
    // Firing (3 > 2).
    let n1 = evaluate_once(&pool, &rules, &mut state, &notifiers, Utc::now()).await;
    assert_eq!(n1, 1);
    ...
    sqlx::query("DELETE FROM logs")
        ...
        .await
        .unwrap();
    let n2 = evaluate_once(&pool, &rules, &mut state, &notifiers, Utc::now()).await;
