---
title: "Sécurité"
description: "Modèle de sécurité, durcissement et auditabilité de niveau HDS."
---
...
- La clé publique configurée est fournie via `[token]` dans `datacat.toml`
  (`public_key_pem` / `public_key_file` / `jwks_url`), le PEM étant passé par référence `${ENV}`
  plutôt qu'écrit en clair — voir [configuration](../configuration/).
- Vérifs : signature, `exp` (+ leeway), `iat`, claims requis (`actor_id`, `session_id`),
  `iss`/`aud` si configurés. Détails : [token](../token/) §7.
...
### 3.5 CORS (§7.4)
- Liste blanche d'origines (`[server.cors].allowed_origins` dans `datacat.toml`). `["*"]` réservé
  au dev (documenté).

### 3.6 Résolution d'IP
- Par défaut, IP du pair TCP (non falsifiable réseau). `X-Forwarded-For` n'est pris en compte
  que si `[server].trust_forwarded_for = true`, à n'activer **que derrière un proxy de confiance
  unique** (sinon l'en-tête est forgeable). On retient alors l'entrée ajoutée par le proxy.
...
### 3.11 Hygiène des secrets (configuration)
- Aucun secret n'est écrit en clair dans `datacat.toml` ni dans `projects/*.toml` : toute chaîne
  peut référencer une variable d'environnement via `${VAR}` (ou `${VAR:-défaut}`), expansée au
  démarrage. Une `${VAR}` requise sans défaut fait **refuser le démarrage** du service
  (fail-closed). Les vrais fichiers de config sont git-ignorés ; seuls les modèles `*.example.toml`
  sont versionnés. Cela maintient les URLs de base, les clés de token, les identifiants S3 et les
  webhooks de notification hors du contrôle de version.
