---
title: "Token"
description: "Le contrat du token d’ingestion et la vérification de signature asymétrique."
---
...
Voir aussi le [contrat d'ingestion](../contract/) §4 (résumé intégré au contrat partagé).
...
d'intégrité du contenu (le contenu des events reste falsifiable — cf. [sécurité](../security/)).
...
|---|---|---|---|
| `iss` | string | recommandé | émetteur (le backend consommateur). Vérifié par Datacat si `[token].issuer` est configuré. |
| `aud` | string | recommandé | audience attendue, ex. `datacat-ingest`. Vérifiée si `[token].audience` est configuré. |
...
La source de la clé publique est configurée dans la section `[token]` de `datacat.toml`. Deux
modes, au choix du déploiement.
...
# ou : public_key_pem = "${TOKEN_PUBLIC_KEY_PEM}"   # "-----BEGIN PUBLIC KEY----- …"
```
...
> En dev/test, un repli par variables d'environnement existe pour ces réglages (`TOKEN_ALG`,
> `TOKEN_PUBLIC_KEY_FILE` / `TOKEN_PUBLIC_KEY_PEM`, `TOKEN_JWKS_URL`, …) en l'absence de
> `datacat.toml`. Voir [configuration](../configuration/).
...
1. En-tête `Authorization: Bearer <jwt>` présent et bien formé (ou champ `token` du corps en
   repli `sendBeacon`, cf. [contrat d'ingestion](../contract/) §1.1).
2. `alg` ∈ {`EdDSA`, `RS256`} (configurable via `[token].algorithms`) — **jamais
   `none`/symétrique**. Sélection de la clé via `kid` (correspondance exacte si fourni).
3. Signature valide contre la **clé publique**.
4. `exp` non dépassé (tolérance `[token].leeway`, défaut 60 s).
5. `iat` présent.
6. Claims requis présents et non vides : `actor_id`, `session_id`.
7. Si configuré : `iss == [token].issuer`, `aud == [token].audience`.
...
C'est le niveau de garantie adéquat pour des analytics tolérantes au bruit. Voir
[sécurité](../security/).
